...
interface ActivityDetailAccountProps {
  chain?: ChainConfig;
  chainType?: ChainType;
...
interface ActivityDetailSheetProps extends ActivityDetailAccountProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  record: DefiRecordRow | null;
}

interface ActivityDetailSheetContentProps extends ActivityDetailAccountProps {
  record: DefiRecordRow;
}

const ActivityDetailSheetContent = memo(
  ({
    chain,
    chainType,
    currentAddress,
    ...
    record,
    walletName,
  }: ActivityDetailSheetContentProps) => {
    const { i18n, t } = useTranslation(['defi', 'global']);
    const pricesQuery = useQueryPrices();

    const actionKey = record.functionName || 'contractCall';
    ...
    const recordAmount = useMemo(
      () => parseActivityRecordAmount({ chain, record }),
      [chain, record],
    );
